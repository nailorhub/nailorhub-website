...
const credibilityItems = [
  ...
  { icon: Layers, text: "No-code & custom code" },
...
export default function Hero() {
  ...
  return (
    <section
    style={{ minHeight: "calc(var(--viewport-height, 100svh) + 124px)" }}
    className="hero-section relative isolate flex flex-col overflow-hidden bg-[#0a1628] pb-[calc(env(safe-area-inset-bottom)+16px)] md:pb-0"
    >
      {/* Tech background */}
      <div
      ...
      {/* Centered content */}
      {/* CHANGE: remove flex-1 so it doesn't steal space from the credibility bar */}
      <div className="relative z-10 flex flex-col items-center justify-center max-w-3xl mx-auto w-full px-6 lg:px-8 pt-16 sm:pt-16 md:pt-12 pb-3">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-0 md:divide-x divide-white/10 items-start">
      {credibilityItems.map((item, i) => (
        ...
        >
          ...
          <span className="pt-[11px] text-[11px] md:text-[12px] font-semibold text-gray-300 leading-tight">
            {item.text}
          </span>
        </motion.div>
      ))}
